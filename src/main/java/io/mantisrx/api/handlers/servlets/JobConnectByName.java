...
public class JobConnectByName implements MantisAPIRequestHandler {

    private static Logger logger = LoggerFactory.getLogger(JobConnectByName.class);
    private final JobSinkConnector sinkConnector;
    public static final String handlerName = "jobconnectbyname";

    public JobConnectByName(MantisClient mantisClient, RemoteSinkConnector remoteSinkConnector, PropertyRepository propertyRepository, WorkerThreadPool workerThreadPool) {
        sinkConnector = new JobSinkConnector(mantisClient, false, remoteSinkConnector, propertyRepository, workerThreadPool);
    }

    @Override
    public String getHelp() {
        return JobConnectByName.handlerName + "/<JobName>";
    }
    ...
    public Observable<Void> handleGet(HttpServerResponse<ByteBuf> response, Request request) {
        ...
        return sinkConnector.connect(PathUtils.getTokenAfter(request.path, getName()), response, request);
    }
