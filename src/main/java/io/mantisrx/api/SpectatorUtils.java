...
public class SpectatorUtils {
    ...
    public static Counter buildAndRegisterCounter(Registry registry, String name, Iterable<Tag> tags) {
        Id id = registry.createId(name).withTags(tags);
        return registry.counter(id);
    }

    public static AtomicDouble buildAndRegisterGauge(Registry registry, String name, Iterable<Tag> tags) {
        Id id = registry.createId(name).withTags(tags);
        return PolledMeter.using(registry).withId(id).monitorValue(new AtomicDouble());
    }

    public static Timer buildAndRegisterTimer(Registry registry, String name, Iterable<Tag> tags) {
        Id id = registry.createId(name).withTags(tags);
        return registry.timer(id);
    }
}
